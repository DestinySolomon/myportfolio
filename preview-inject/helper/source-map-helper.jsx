...
/**
 * Opening tag information (documented shape)
 * { tagName, lineIndex, columnStart, columnEnd, isSelfClosing, closeLineIndex, closeColumnEnd }
 */
 ...
 */
export function correctSourceMapPosition(lines, targetIndex, targetColumn) {
  ...
  if (isOnlyClosingTag || endsWithClosingTag || isAfterClosingTag) {
    ...
    if (targetColumn != null) {
      ...
      if (
        nextOpenMatch &&
        afterTarget[(nextOpenMatch.index ?? 0) + 1] !== "/"
      ) {
        return {
          lineIndex: targetIndex,
          column: targetColumn + (nextOpenMatch.index ?? 0) + 1,
        };
      }
    }
    ...
    for (
      ...
      if (openTagMatch && nextLine[(openTagMatch.index ?? 0) + 1] !== "/") {
        // Found an opening tag on this line
        return { lineIndex: i, column: (openTagMatch.index ?? 0) + 1 };
      }
    }
  }
 ...
 */
export function findTagEnd(lines, startLineIndex, startColumn) {
  let depth = 0;
 ...
 */
export function findClosingTag(lines, tagName, startLineIndex, startColumn) {
  let depth = 1;
 ...
 */
export function findOpeningTag(lines, targetLineIndex, targetColumn) {
  let bestMatch;
 ...
 */
export function findNextMatchingTag(lines, startLineIndex, expectedTagName) {
 ...
 */
export function extractRange(lines, startLine, startCol, endLine, endCol) {
...
export function extractSourceContext(
  ...
  for (let i = startLine; i <= endLine; i++) {
    ...
    const formattedLine = `${prefix} ${lineNumber.toString().padStart(4, " ")} | ${lines[i] || ""}`;
